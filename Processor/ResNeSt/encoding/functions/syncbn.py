    ...
    def backward(ctx, dz):
        ...
        if dz.is_cuda:
            dx, _dex, _dexs, dgamma, dbeta = \
                cpu.batchnorm_backward(dz, x, _ex, _exs, gamma, beta, ctx.eps)
        ...
        if ctx.training:
            ...
            if x.is_cuda:
                dx_ = cpu.expectation_backward(x, _dex, _dexs)
...
def _act_forward(ctx, x):
    if ctx.activation.lower() == "leaky_relu":
        if x.is_cuda:
            cpu.leaky_relu_forward(x, ctx.slope)
...
def _act_backward(ctx, x, dx):
    if ctx.activation.lower() == "leaky_relu":
        if x.is_cuda:
            cpu.leaky_relu_backward(x, dx, ctx.slope)
