...
class DIODE(Dataset):
    ...
    def __getitem__(self, index):
        ...
        im_label = im_fname.split("\\")[7]  # get indoors / outdoors
        im_name = im_fname.split("\\")[10]  # get image_name (label)

        de = np.load(de_fname).squeeze()
        de_mask = np.load(de_mask_fname)
        return im, de, de_mask, im_label, im_name
...
if __name__ == '__main__':
    ...
    for im, dm, mask, label, name in dset, "computing mean rgb", len(dset):  # for each image in the dataset...
        pass
